/////////////////////////////////////////////////////////////////////////
//  Name: MazeGen300.cpp
//  Purpose: Generate a 10x30 maze in a format that can be read from a
//           data file by a TRS-80 Model 100 BASIC program. 
//           
//  Created by L. Johnson  7/8/24 from MazeGen.cpp that was written for
//                     the Altar 8800 MAZE029 assembly language program.
//  
/////////////////////////////////////////////////////////////////////////
...
int main(int argc, char** argv) {
  ...
  int availRooms[300] ; // Will be populated just prior to picking a random room
  ...
  while (exitFoundFlag == FALSE)  {
    ...
    // Check north 
    if (wIndex>29) { // Room is not on the north border of maze
    ...
    // Check South 
    if (wIndex<270) { // Room is not on the south border of maze
    ...
    // Check East 
    if ((wIndex+1) %30 != 0) { // Room is not on the east border of maze
    ...
    // Check West 
    if (wIndex %30 != 0) { // Room is not on the west border of maze
    ...
    if (exitCnt>0) {  // Choose a direction randomly from the availDirections
      ...
      case 8:  // North
        ...
        wIndex = wIndex - 30;   // Set new working index
  ...
  */
  ...
  for (j=0;j<10;j++) {
    ...
    for (i=0;i<30;i++) {
      if ((maze[i+j*30] & 0x01)==0) printf("|"); else printf(" ");
      if ((maze[i+j*30] & 0x10)==0x10) printf(" S "); 
      if ((maze[i+j*30] & 0x80)==0x80) printf(" F "); 
      if ((maze[i+j*30] & 0x90)==0) printf("   ");
    }
    printf("|\n") ;
  
    // Bottom of Cell  (South Exits)
    for (i=0;i<30;i++) {
      if ((maze[i+j*30] & 0x04)==0) printf("+---"); else printf("+   ");
    }
    printf("+\n") ; 
  }
  
  
    //  Output maze array as BASIC data statements (one nibble per cell)
  j=1000; //Initial line number 
  for (i=0;i<300;i++){
    if (((i)%30)==0) {
      printf("%4d DATA ",j);
	  j=j+10;	
	}
    printf("%d",(0x000f & maze[i]));
    if (((i+1)%30)==0) printf("\n"); else printf(",");
  }
